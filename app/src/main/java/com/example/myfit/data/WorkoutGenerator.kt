...
interface WorkoutGenerator {
    /**
     * 1. What: Produces a 7-day plan tailored to [prefs]; [recentHistory] is optional extra
     *    context (most-recent-first summaries) to improve quality.
...
//        runCatching {
//            val response = model.generateContent(buildPrompt(prefs, recentHistory))
...
//    }
    override suspend fun generate(
        ...
        runCatching {
            ...
            android.util.Log.e("WorkoutAI", "Gemini generation failed", it)  // ← add
        }
    }
     ...
     */
    private fun buildPrompt(prefs: OnboardingPreferences, recentHistory: List<String>): String {
        ...
        return """
            ...
            - "day" MUST be exactly one of: "Mon","Tue","Wed","Thu","Fri","Sat","Sun", in that order.
...
object MockWorkoutGenerator {
    ...
    /**
     * 1. What: Builds a 7-day [WorkoutPlan] with [trainingDays] workout days evenly spaced across
     *    the week and the rest as rest days.
     ...
     */
    fun generate(trainingDays: Int = 4): WorkoutPlan {
        val days = WorkoutPlan.DAY_ORDER.size
        ...
        var rotationIdx = 0
        val plan = WorkoutPlan.DAY_ORDER.mapIndexed { index, label ->
